...
/* PC13: botón de usuario de la Nucleo-L476RG. Para cambiar de pin:
 * - Modifica la letra ('C') y el número (13) aquí.
 * - No requiere cambios en el overlay para GPIO simples de entrada. */
...
bool power_status_manager_init(void)
{
	if (!gpio_is_ready_dt(&button)) {
		LOG_ERR("GPIO del pulsador no listo — verificar nodo user_button en overlay");
		return false;
	}
	...
	LOG_INF("Pulsador listo en %s pin %d", button.port->name, button.pin);
	return true;
}
